...
(function ($, undefined) {
    ...
    $.widget("cg.combogrid", {
        options: {
            ...
            mode: "remote",
            data: null,
            isSecondCascade: false,
            secondCascadeValue: null,
            secondCascadeModel: null,
            isProductNo:false,
        },
        source: null,
        lastOrdered: "",
        ...
        _create: function () {
            ...
            this.options.source = function (request, response) {
                if (this.options.mode != "local") {
                    ...
                    response(1, 1, $.map(this.options.data.rows, function (item) {
                        return item;
                    }));
                }
            };
        ...
        _initSource: function () {
            ...
            if ($.isArray(this.options.source)) {
                ...
                this.array = this.options.source;
                this.source = function (request, response) {
                    //array = this.array(request, response);
                    if (request.term != "") {
                        response(1, 1, $.map($.cg.combogrid.filter(this.options.data.rows, request.term, this.options.filtModel, this.options.isSecondCascade, this.options.secondCascadeModel, this.options.secondCascadeValue), function (item) {
                            return item;
                        }));
                    }
                    else if (this.options.isSecondCascade) {
                        response(1, 1, $.map($.cg.combogrid.filter(this.options.data.rows, request.term, this.options.filtModel, this.options.isSecondCascade, this.options.secondCascadeModel, this.options.secondCascadeValue), function (item) {
                            return item;
                        }));
                    }
        ...
        _renderHeader: function (ul, colModel) {
            var self = this;
            div = $('<div id="cg-divHeader" class="ui-state-default" style="width:' + self.options.width + self.options.munit + '">');
        ...
        _renderMenu: function (ul, items, colModel) {
            var self = this;
            $(".cg-menu").append('<div class="subItem"></div>');
        ...
        _renderItem: function (ul, item, colModel) {
            var self = this;
            this.rowNumber++;
            div = $('<div class="cg-colItem" style="width:' + self.options.width + self.options.munit + '">');
            $.each(colModel, function (index, col) {
                if (col.width == undefined) { col.width = 100 / colModel.length; }
                if (col.align == undefined) { col.align = "center"; }
                var hide = "";
            ...
            // }
            $("<div class='cg-comboItem'></div>").data("item.combogrid", item).append(div).appendTo($(".subItem"));
        },
    ...
    $.extend($.cg.combogrid, {
        ...
        filter: function (array, term, filtModel, isSecondCascade, secondCascadeModel, secondCascadeValue) {
            //var matcher = new RegExp($.cg.combogrid.escapeRegex(term), "gi");
            var filtValue = $.cg.combogrid.escapeRegex(term);
            return $.grep(array, function (value) {
                var isFind = false;

                if (filtModel != null && filtModel.length > 0 && term != "") {
                    for (var i = 0; i < filtModel.length; i++) {
                        if (value[filtModel[i].columnName] != null) {
                            if (value[filtModel[i].columnName].toString().toUpperCase().indexOf(filtValue.toUpperCase()) > -1) {
                ...
                if (isFind && isSecondCascade && secondCascadeModel != null && secondCascadeModel.length > 0 && secondCascadeValue != null) {
                    isFind = false;
                    //判断是否是对象，如为对象，需要获取属性；否则为属性
                    if (value[secondCascadeModel[0].obejctName] != null && typeof value[secondCascadeModel[0].obejctName] === "object") {
                        var object = value[secondCascadeModel[0].obejctName];
                        var property = object[secondCascadeModel[0].propertyName];
                        if (property.toString().indexOf(secondCascadeValue) > -1) {
                            isFind = true;
                        }
                    }
                    else {
                        var property = value[secondCascadeModel[0].obejctName];
                        if (property.toString().indexOf(secondCascadeValue) > -1) {
                            isFind = true;
                        }
                    }
...
(function ($) {
    ...
    $.widget("cg.menucombo", {
        ...
        refresh: function () {
            ...
            var items = this.element.children("div:not(.cg-menu-item):not(#cg-divHeader):not(.cg-comboButton):has(div)").children()
            ...
            //     .end();
            //     // this._trigger("mouseenter", event, { item:  });
            //     this.activate(event, $(items[0]));
            // }
        },
        ...
        move: function (direction, edge, event) {
            if (!this.active) {
                this.activate(event, this.element.children('.subItem').children(edge));
                return;
            }
            var next = this.active[direction + "All"](".subItem .cg-menu-item").eq(0);
            var item = next;
            if (next.length) {
                this.activate(event, next);
            } else {
                this.activate(event, this.element.children('.subItem').children(edge));
                item = this.element.children('.subItem').children(edge);
            }
        },
